...
var MTextillate = function () {
    ...
    MTextillate.prototype.in = function _in() {
        ...
        var thisOptions = deepmerge_1.all([options, jDom.getDataOpt(nextReferEle)]);
    ...
    MTextillate.prototype.out = function out(cb) {
        ...
        var thisOptions = jDom.dataApi.get(nextReferEle, 'bindOptions');
        var isOutEffectV = isOutEffect(thisOptions.out.effect);
...
var _textillate = {
    entry: function entry(settings) {
        ...
        arrayEach.call(outerEles, function (el, index) {
            var options = deepmerge_1.all([MTextillate.defaultConfig, jDom.getDataOpt(el), (typeof settings === 'undefined' ? 'undefined' : _typeof(settings)) === 'object' && settings]);
